            ...
            case "MODE":
                ...
                modeList.forEach(function(mode) {
                    ...
                    if ( mode in self.prefixForMode ) {
                        ...
                        var modeArg = undefined;
                        // channel modes
                        if ( adding ) {
                            if ( self.supported.channel.modes['d'].indexOf(mode) === -1 ) {
                                modeArg = modeArgs.shift();
                            } 
                            if ( self.supported.channel.modes['a'].indexOf(mode) === -1 && channel.mode.indexOf(mode) === -1 )
                                channel.mode += mode;

                            self.emit('+mode', message.args[0], message.nick, mode, modeArg, message);
                        }
                        else {
                            if ( self.supported.channel.modes['d'].indexOf(mode) === -1 && self.supported.channel.modes['c'].indexOf(mode) === -1 ) {
                                modeArg = modeArgs.shift();
                            } 
...
function parseMessage(line, stripColors) {
    ...
    if (message.prefixIsUserHostmask()) {
        ...
        message.nick = hostmask.nickname;
        message.user = hostmask.username;
        message.host = hostmask.hostname;
    } else {
